...
class xLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, embedding_dim, device=th.device('cuda:0')):
        super().__init__()
        self.device = device
        # output, value, key, query
        self.multilinear_gates = MultiLinear((hidden_size * 4, input_size, embedding_dim), 'bi, lij -> blj',
                                             bias_shape=(hidden_size * 4, embedding_dim)).to(self.device)
        # input, forget
    ...
    def reset_states(self, batch_size: int):
        ...
        th.zeros((batch_size, self.hidden_size, self.embedding_dim, self.embedding_dim), dtype=th.float32).to(
            self.device))
        self.m, self.n = (th.zeros((batch_size, self.hidden_size), dtype=th.float32).to(self.device),
                          th.zeros((batch_size, self.hidden_size, self.embedding_dim), dtype=th.float32).to(
                              self.device))
    ...
    def _forward_step(self, input: th.Tensor):
        ...
        self.m = m
        self.cell_state = (f.unsqueeze(-1).unsqueeze(-1) * self.cell_state) + i.unsqueeze(-1).unsqueeze(-1) * th.einsum(
            'bli, blj -> blij', v, k)
        self.n = f.unsqueeze(-1) * self.n + i.unsqueeze(-1) * k
        self.hidden_state = o * th.einsum('blij, bli -> blj', self.cell_state, q) / th.clamp(th.sum(self.n * q, dim=-1),
                                                                                             1, th.inf).unsqueeze(-1)
    def forward(self, input: th.Tensor):
